...
class ClientHandler(Resource):
    def post(self):
        ...
        if not Client.query.filter(Client.phone == сlient_phone).first():
            ...
            client_db.save()
            api_info = (
                Client.query.filter(Client.phone == сlient_phone).first().to_dict()
            )
        api_reply = response_processing(answer_code, api_info)
        return jsonify(api_reply)
    ...
    def delete(self):
        ...
        answer_code = "02"
        client_info_from_request = request.get_json()
        client_db = Client.query.filter(
            Client.id == client_info_from_request.get("id")
        ).first()
        if client_db:
            answer_code = "00"
            client_db.delete()

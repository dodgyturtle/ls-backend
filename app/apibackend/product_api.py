...
class ProductHandler(Resource):
    def post(self):
        ...
        request_productname = product_info_from_request.get("nameproduct")
        if not Product.query.filter(Product.nameproduct == request_productname).first():
            answer_code = "11"
            product_db = Product(**product_info_from_request)
            product_db.save()
            api_info = Product.query.filter(
                Product.nameproduct == request_productname).first().to_dict()
        api_reply = response_processing(answer_code, api_info)
        return jsonify(api_reply)
    ...
    def delete(self):
        ...
        product_db = Product.query.filter(
            Product.id == product_info_from_request.get("id")
        ).first()
        if product_db:
            answer_code = "00"
            product_db.delete()

...
def filter_for_table_between_date(base, startdate: datetime, finishdate: datetime, client_id = None):
    finishdate = finishdate + timedelta(days=1)
    if client_id:
        information_for_table = db.session.query(base).filter(
            Sale.client_id == client_id).order_by(

...
from app.webbackend.forms import (
    AddClientForm,
    AddComingForm,
    AddProductForm,
    AddSaleForm,
...
from app.webbackend.handlers import (
    errors_convert_dict_to_string,
...
def product():
    ...
    if form.submit.data and form.validate_on_submit():
        ...
        flash(
            f"Ошибка ввода данных: { errors_convert_dict_to_string(form.errors) }",
            "danger",
        )
        return redirect(url_for("webbackend.product"))
...
def productupdate(product_id: int):
    ...
    return render_template(
        "webbackend/productupdate.html", form=form, product=product_db
    )
...
def productdelete(product_id: int):
    ...
    if form.is_submitted():
        if form.submit.data and product_db:
            ...
            return redirect(url_for("webbackend.product"))
    return render_template(
        "webbackend/productdelete.html", form=form, product=product_db
    )
    ...
    if form_table.is_submitted():
        if form_table.showdata.data:
            ...
            if startdate and finishdate:
                ...
                flash(f"Ошибка ввода данных: Неверный формат даты!", "danger")
    return render_template(
        "webbackend/client.html", form=form, form_table=form_table, clients=clients
    )
...
def sale():
    ...
    form.client_id.choices = [
        (client.id, client.fullname) for client in Client.query.all()
    ]
    form.product_id.choices = [(0, "Выберите товар")]
    [
        form.product_id.choices.append((product.id, product.nameproduct))
    ...
    if form_table.is_submitted():
        if form_table.showdata.data:
            ...
            if startdate and finishdate:
                ...
                flash(f"Ошибка ввода данных: Неверный формат даты!", "danger")
    return render_template(
        "webbackend/sale.html",
        form=form,
        form_table=form_table,
        sales=sales,
        all_price=all_price_json,
    )
...
def saleupdate(sale_id: int):
    ...
    form.product_id.choices = [
        (product.id, product.nameproduct) for product in Product.query.all()
    ]
    ...
    if form_table.is_submitted():
        if form_table.showdata.data:
            ...
            if startdate and finishdate:
                ...
                flash(f"Ошибка ввода данных: Неверный формат даты!", "danger")
    return render_template(
        "webbackend/coming.html", form=form, form_table=form_table, comings=comings
    )
...
def balanceupdate(balance_id: int):
    ...
    if form.is_submitted():
        if form.submit.data and form.validate():
            ...
            return redirect(url_for("webbackend.balance"))
    return render_template(
        "webbackend/balanceupdate.html", form=form, balance=balance_db
    )
...
def balancedelete(balance_id: int):
    ...
    if form.is_submitted():
        if form.submit.data and balance_db:
            ...
            return redirect(url_for("webbackend.balance"))
    return render_template(
        "webbackend/balancedelete.html", form=form, balance=balance_db
    )
    ...
    if form_table.is_submitted():
        if form_table.showdata.data:
            ...
            if startdate and finishdate:
                ...
                flash(f"Ошибка ввода данных: Неверный формат даты!", "danger")
    return render_template(
        "webbackend/stock/stock.html", form=form, form_table=form_table, stocks=stocks
    )
...
def stockupdate(stock_id: int):
    ...
    if form.is_submitted():
        if form.submit.data and form.validate():
            ...
            return redirect(url_for("webbackend.stock"))
    return render_template(
        "webbackend/stock/stockupdate.html", form=form, stock=stock_db
    )
...
def stockdelete(stock_id: int):
    ...
    if form.is_submitted():
        if form.submit.data and stock_db:
            ...
            return redirect(url_for("webbackend.stock"))
    return render_template(
        "webbackend/stock/stockdelete.html", form=form, stock=stock_db
    )
...
def order(client_id: int):
    form_table = FilterTable(request.form)
    orders_db = (
        Sale.query.filter(Sale.client_id == client_id).order_by(Sale.date.desc()).all()
    )
    if form_table.is_submitted():
        if form_table.showdata.data:
            startdate = form_table.startdate.data
            finishdate = form_table.finishdate.data
            if startdate and finishdate:
                orders_db = filter_for_table_between_date(
                    Sale, startdate, finishdate, client_id
                )
            else:
                flash(f"Ошибка ввода данных: Неверный формат даты!", "danger")
    return render_template(
        "webbackend/order/order.html",
        form_table=form_table,
        orders=orders_db,
        client_id=client_id,
    )
